...
import { createSummaryRoute } from "./summary";
...
describe("POST /api/articles/:id/summary", () => {
  ...
  it("キャッシュがない場合は要約を生成して201を返すこと", async () => {
    ...
    mockSummarizeArticle.mockResolvedValueOnce({
      summary: "生成された要約テキスト",
      model: "qwen3.5-9b",
    });

    const mockInsertedSummary = {
      id: "summary_new",
      articleId: MOCK_ARTICLE.id,
      language: "ja",
      summary: "生成された要約テキスト",
    ...
    mockInsertReturning.mockResolvedValueOnce([mockInsertedSummary]);
