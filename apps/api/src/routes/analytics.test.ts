...
describe("createAnalyticsRoute", () => {
	describe("POST /events", () => {
		it("有効なイベントを送信して201を返すこと", async () => {
			// Arrange
			const mockReturn = { id: "test-id", userId: TEST_USER.id, event: "article_view", properties: "{}", createdAt: "2026-01-01" };
		...
		it("insertが呼ばれること", async () => {
			// Arrange
			const mockReturn = { id: "test-id", userId: TEST_USER.id, event: "search", properties: "{}", createdAt: "2026-01-01" };
			const db = createMockDb(mockReturn);
			const app = createTestApp(db, TEST_USER);
