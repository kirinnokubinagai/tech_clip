...
新設計（4 体セット固定 spawn）では、ui-reviewer は **常に e2e-reviewer から `e2e-approved: <hash>` を受信** する。ui-designer からの `impl-ready` を直接受け取ることはない（CONFLICT_RESOLVED は ui-designer 直送のまま）。

理由: e2e-reviewer がフェーズ 0 で `evaluate-paths.sh` を実行し、E2E 影響なしなら短絡してそのまま ui-reviewer に `e2e-approved` を転送する。E2E 影響ありなら maestro 全 flow PASS 後に `e2e-approved` を送る。

### 複数レーン時の集約

`issue-{N}-ui-designer-api` / `issue-{N}-ui-designer-mobile` のように同一 Issue で複数の lane 付き ui-designer がいる場合:

- 各 lane の ui-designer は **e2e-reviewer に** `impl-ready: <hash> lane={lane-name}` を送る
- e2e-reviewer は全 lane を集約してから shard 実行（または短絡）し、ui-reviewer に **1 通だけ** `e2e-approved: <hash>` を送る
- ui-reviewer から見ると単独モードと同じく `e2e-approved` 1 通で開始判定する

### フェーズ 0: e2e-reviewer / ui-designer からの SendMessage 待機

以下のメッセージを待つ:

- **`e2e-approved: <commit-hash>`**（e2e-reviewer から、通常はこれが入口）
- `CONFLICT_RESOLVED: <commit-hash>`（ui-designer から、conflict 解消後）
- `ABORT: <理由>`（ui-designer から、本 Issue の変更不要判定時）
...
if [ "$LOCAL_HASH" != "$APPROVED_HASH" ]; then
  SendMessage(to: "issue-{issue_number}-ui-designer", "ERROR: e2e-approved hash ($APPROVED_HASH) が local HEAD ($LOCAL_HASH) と一致しません。正しい commit hash で e2e-reviewer に impl-ready を再送信してください。")
  exit 0
fi
...
if [ -n "$UNCOMMITTED" ]; then
  SendMessage(to: "issue-{issue_number}-ui-designer", "ERROR: uncommitted changes が存在します。すべての変更を commit してから e2e-reviewer に impl-ready を再送信してください。")
  exit 0
fi
...
```
送信後、フェーズ 0 に戻り analyst → ui-designer → e2e-reviewer → e2e-approved を待つ。
...
4. **問題あり**:
   - `SendMessage(to: "issue-{issue_number}-ui-designer", "CHANGES_REQUESTED: 解消結果に問題があります: <具体的な指摘>")`
   - フェーズ 0 に戻る
5. **問題なし**:
...
自分で UI/UX レビューを実施して判定を行う。自動 claude-review bot の判定を待たない。

1. フェーズ 3 の UI/UX レビュー観点で自分がレビューを行い合否を判定する
2. PASS の場合:
   ```bash
...
3. CHANGES_REQUESTED の場合: `SendMessage(to: "issue-{issue_number}-ui-designer", "CHANGES_REQUESTED: <指摘内容>")` してフェーズ 0 へ戻る
...
送信後、フェーズ 0 に戻り次の e2e-approved を待機する。
...
```bash
# レビュー通過マーカー作成
bash {worktree}/scripts/gate/create-review-marker.sh --agent issue-{N}-ui-reviewer
...
SendMessage(to: "issue-{issue_number}-ui-designer", "CHANGES_REQUESTED: $BOT_BODY")
rm -f "$POLLING_DIR/pr-${PR_NUMBER}.json"
# フェーズ 0 に戻る
```
...
3. 手動でコード・UI/UX レビューを実行する
4. 問題なければ `bash {worktree}/scripts/gate/create-review-marker.sh --agent issue-{N}-ui-reviewer` を実行する
...
- 通常フローの範囲内の作業（UI/UX レビュー、push、PR 作成、GitHub ポーリング、SendMessage 等）
