...
if [ "$LOCAL_HASH" != "$IMPL_READY_HASH" ]; then
  SendMessage(to: "issue-{issue_number}-ui-designer", "ERROR: impl-ready hash ($IMPL_READY_HASH) が local HEAD ($LOCAL_HASH) と一致しません。正しい commit hash を送信してください。")
  exit 0
fi
...
if [ -n "$UNCOMMITTED" ]; then
  SendMessage(to: "issue-{issue_number}-ui-designer", "ERROR: uncommitted changes が存在します。すべての変更を commit してから impl-ready を送信してください。")
  exit 0
fi
