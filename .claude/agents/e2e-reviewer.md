...
B. 通常実行フロー:
   ...
   2. adb devices で接続中 emulator を確認し、shard N に割り当てる emulator を決定する
      （shard1 → 最初の emulator、shard2 → 2番目、など）
   3. Bash(run_in_background=true):
        bash scripts/gate/run-maestro-and-create-marker.sh --agent <name> --shard <N>/<TOTAL> --device emulator-XXXX
   4. 監視ループ（~30秒おき）:
...
監視ループは **agent (Skill ツール実行) の制御フロー** で回す。Bash の `until` / `while` ループ内で監視を行うと SendMessage が打てなくなるため禁止。

```
0. adb devices で接続中の emulator 一覧を取得し、shard N に対応する emulator を割り当てる
   例: emulators=(emulator-5554 emulator-5556 emulator-5558)
       shard1 → emulators[0], shard2 → emulators[1], ...

1. Bash(run_in_background=true):
     bash scripts/gate/run-maestro-and-create-marker.sh \
       --agent <name> --shard <N>/<TOTAL> --device <emulator-XXXX>
   → background_task_id を受け取る
   ※ --device を必ず指定する。省略すると複数 emulator 環境で誤った emulator に接続する。

2. RESULT_XML パスを計算（スクリプト内で生成される名前と同じ規則）:
     /tmp/maestro-result-<HEAD_SHA8>-<TIMESTAMP>-shard<N>of<TOTAL>.xml

3. 監視ループ（agent 制御、最大 60 iteration = 30 分相当）:

   for i in 1..60:
     a. Read(<RESULT_XML>) を試みる（ファイルが存在しなければスキップ）
     b. 前回確認時より増えた <testcase> 要素があれば、各 testcase ごとに
        SendMessage(to: "team-lead", "STATE_UPDATE: ... flow <flow_name> PASS/FAIL")
        を送信する。**1 iteration で複数 SendMessage を発行してよい**。
     c. Read(.claude/.e2e-shard-<N>of<TOTAL>.json) を試みる
     d. "status" キーが存在すれば完了:
        - SendMessage(to: "team-lead", "STATE_UPDATE: ... shard X/N completed (PASS/FAIL, ...)")
        - ループを抜ける
     e. 完了していなければ Bash(`sleep 30`, run_in_background=false) で 30 秒待機。
        ここでは Bash 1 コマンドのみ（30 秒以内に確実に戻る）。SendMessage を含めない。
     f. 次 iteration へ

4. 60 iteration 完了しても "status" 未確定なら:
   SendMessage(to: "team-lead", "STATE_UPDATE: ... shard X/N TIMEOUT (30 min)")
   → 失敗扱いで次フェーズ（CHANGES_REQUESTED）へ進む

5. 完了後は次ステップ（aggregate または reviewer 通知）へ進む
```

### 監視ループの実装制約（必読）

- **Bash の `until` / `while` ループ内に監視ロジックを書かない**。Bash がブロッキングのため、ループ内に長時間滞留すると agent が SendMessage を発行できなくなる
- **`sleep 30` を Bash で実行する場合は 30 秒以内に必ず戻る単発コマンド**として呼ぶ。SendMessage は agent の skill ループ側で発行する
- agent は監視ループ 1 iteration 終わるごとに skill ツールに戻り、Read + SendMessage を発行できるようにする
