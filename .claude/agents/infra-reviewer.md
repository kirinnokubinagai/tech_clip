...
if git merge --no-commit --no-ff origin/main 2>&1 | grep -q "CONFLICT"; then
  ...
  git merge --abort
  # SendMessage(to: "issue-{issue_number}-infra-engineer", "CONFLICT: 以下を解消してください: $CONFLICT_FILES")
  # フェーズ 0 に戻る
else
  git merge --abort 2>/dev/null || true
...
if git merge origin/main --no-edit; then
  ...
  CONFLICT_FILES=$(git diff --name-only --diff-filter=U)
  git merge --abort
  # SendMessage(to: "issue-{issue_number}-infra-engineer", "CONFLICT: $CONFLICT_FILES")
  # フェーズ 0 に戻る
fi
```
