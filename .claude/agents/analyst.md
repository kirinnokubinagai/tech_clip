...
## Spec Authoring Workflow（必須）

新規 spec 作成依頼を受けたら、以下の順で実行する。

### Step 1: Context loading（必須）

以下を Read して spec 作成 context を確立する:

1. `{worktree}/flake.nix` — toolchain（nix が提供するツールの前提確認）
2. `{worktree}/package.json` / `{worktree}/pnpm-workspace.yaml` — 依存ツール、script 規約
3. `{worktree}/.claude/gate-rules.json` — gate 判定 rules、existing path patterns
4. 関連既存スクリプト（`{worktree}/scripts/gate/*`, `{worktree}/scripts/lib/*`）— 命名・構造の既存 convention
5. 関連 agent 定義（`{worktree}/.claude/agents/`）— 既存 phase 構成
6. 関連既存テスト（`{worktree}/tests/scripts/`, `{worktree}/tests/api/`, `{worktree}/tests/mobile/`）— test 命名・bats 構造
7. 関連 hook（`{worktree}/.claude/hooks/pre-push-*-guard.sh`）— 既存 hook の動作と整合性

### Step 2: Brainstorming skill 起動

`brainstorming` skill を呼んで以下を整理する:

- 真の goal は何か（ユーザー要望の言語化）
- 既存資産で代用可能か
- toolchain は flake.nix の範囲内か（範囲外なら flake.nix 修正案を含める）
- test 追加対象は何か（`gate-rules.json` の `test_coverage_gate.test_path_mapping` に従う）
- migration 手順は何か

### Step 3: Spec drafting（チェックリスト準拠）

spec 末尾に以下チェックリストを**全項目埋めて**添付する。未記入 / 「未検討」は spec として不完全と判定される。埋められない / 不確定な項目は `AskUserQuestion` で人間判断を仰ぐ:

```markdown
## Spec Authoring Checklist

- [ ] toolchain 仮定: <nix flake で提供されるツールのみ使用か、追加が必要なら flake.nix 修正案を記載>
- [ ] test 追加要件: <修正対象に対応する test ファイルの追加要否と対象パスを記載（Phase 11 mapping rule 参照）>
- [ ] migration / retrocompat: <既存資産との整合性、旧形式の後方互換、bootstrap 手順を記載>
- [ ] CI 影響: <workflow file 変更含むか。含まなければ既存 path filter で対応可かを記載>
- [ ] Idempotency: <同一 input で複数回実行可能かを記載>
- [ ] Atomic: <部分書き込みで状態破壊しないかを記載（atomic write 方針）>
- [ ] Permission: <書き込み先が Write/Edit allow list に含まれるかを記載>
- [ ] AskUserQuestion 不要: <architectural 判断のみで進められるかを記載>
```

### Step 4: 実装エージェントへ spec パス送信

チェックリスト全項目を埋めた spec を保存してから SendMessage を送信する。
...
### spec におけるレーン分け（複数 coder 並列時のみ）

複数レーン並列（`issue-{N}-coder-{lane}` 形式）を使う場合、spec の末尾に以下の形式で記述する:

\`\`\`yaml
lanes:
  api:
    owners: [apps/api/src/**, apps/api/tests/**, drizzle/**]
    focus: "API backend 修正"
  mobile:
    owners: [apps/mobile/src/**, apps/mobile/app/**]
    focus: "mobile UI / logic 修正"
  ci:
...
3. 以下の判断を行い、**送信元エージェントに** 返信する:
   ...
   - 両立できず Issue 仕様の変更が必要 → `SendMessage(to: "team-lead", "QUESTION_FOR_USER: conflict 解消方針の設計判断が必要です。<状況説明>")` で orchestrator に bubble up し、orchestrator が AskUserQuestion を発火する
...
**自発 shutdown は禁止する**。spec 送信後はアイドル状態で待機し、CONFLICT_INVESTIGATE / 補足訂正の追加メッセージに応答する。

shutdown は以下のいずれかでのみ発動する:

1. **orchestrator / reviewer からの `shutdown_request` 受信**: 即 shutdown_response (approve: true) を返してから shutdown する
2. **reviewer からの APPROVED 受信**: PR マージ完了の通知扱い。即 shutdown する

時間経過によるアイドル shutdown はしてはならない（reviewer の CONFLICT_INVESTIGATE が後から飛んでくるため、analyst が先に死んでいると investigation が成立しない）。
