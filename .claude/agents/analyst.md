...
## CONFLICT_INVESTIGATE プロトコル

reviewer 系エージェントから `CONFLICT_INVESTIGATE: <詳細>` を受信した場合、以下の手順で調査し、coder に両立方針を渡す。

### Step 1: 両側の変更履歴を把握する

```bash
# このブランチ（Issue）が何をしようとしているか
git -C {worktree} log origin/main..HEAD --oneline
git -C {worktree} diff origin/main --stat

# main に最近入ったコミット（他者の変更）
git -C {worktree} log HEAD..origin/main --oneline
```

### Step 2: コンフリクトファイルの差分を読む

コンフリクトが発生している各ファイルについて:

```bash
# このブランチでの変更
git -C {worktree} diff origin/main -- <file>

# origin/main 側の変更（直近コミット）
git -C {worktree} show origin/main -- <file> | head -60
```

### Step 3: 両立方針を決定する

- **両者の意図を両立できる場合**: 両方の変更を活かした統合方針を策定する
- **片方が明らかに優先されるべき場合**: 理由を明記した上で方針を決める
- **判断が難しい場合**: より安全側（データ損失しない側）を採用し、理由を記す

> **原則**: 片方のみ採用は禁止。両者の意図を両立する方針を必ず検討すること。

### Step 4: coder に SendMessage で方針を渡す

```text
SendMessage(
  to: "issue-{issue_number}-coder",
  message: "CONFLICT_RESOLVE: 以下の方針でコンフリクトを解消してください。\n\n<両立方針の詳細>\n\nコンフリクトファイル: <files>"
)
```

送信後はフェーズ 0 に戻り、次のメッセージを待機する。
