...
3. spec の方針で解消できない箇所がある場合:
   - `SendMessage(to: "issue-{N}-analyst", "CONFLICT_INVESTIGATE: <状況説明>")` を送信
   - analyst からの `CONFLICT_RESOLVE_DESIGN:` 応答を受信するまで待機
   - 応答の方針を適用してから解消を完了
   - `CONFLICT_RESOLVE_DESIGN:` に "不要" が含まれる場合（本 Issue の変更が main で不要と判定）:
     `SendMessage(to: "issue-{N}-reviewer", "ABORT: CONFLICT_INVESTIGATE の結果、本 Issue の変更は不要と判断されました。<analyst の理由>")` を送信
4. `git commit` する（push しない）
5. `SendMessage(to: "issue-{N}-reviewer", "CONFLICT_RESOLVED: <commit-hash>")` を送信

### reviewer の解消結果監査

1. 解消 commit の diff を読む（`git show <commit-hash>`）
2. 監査ポイント: 片側採用になっていないか、両側の意図が両立されているか
3. 問題あり → `SendMessage(to: "issue-{N}-coder", "CHANGES_REQUESTED: <理由>")` → coder がやり直し
