...
| 操作 | コマンド |
|------|---------|
| ブランチ作成 | `scripts/create-worktree.sh <issue-number> <short-desc>` |
...
| `git --git-dir=...` / `GIT_DIR=...` / `GIT_WORK_TREE=...` | worktree と index を誤って壊しやすい |
...
```bash
# REPO_ROOT: mainブランチのリポジトリルート
REPO_ROOT=$(git -C /Users/.../tech_clip/main rev-parse --show-toplevel)
# WORKTREE_BASE: mainと兄弟のディレクトリ
WORKTREE_BASE=$(dirname "$REPO_ROOT")

# 作成（内部で fetch / direnv allow / pnpm install まで実行）
scripts/create-worktree.sh N short-desc

# 作業完了後のクリーンアップ（マージ済みの場合のみ）
git -C "$REPO_ROOT" worktree remove "${WORKTREE_BASE}/issue-N"
git -C "$REPO_ROOT" branch -d issue/N/short-desc
```

- worktree は main と同じ親ディレクトリに兄弟として作成
- ブランチ名は `issue/<番号>/<kebab-case説明>`
- Git 操作は `cd <worktree>` または `git -C <worktree-path> ...` を使う
- `pnpm` / `node` / `biome` / `turbo` は `cd <worktree> && direnv exec <worktree-path> ...` を優先する
- `--git-dir` / `GIT_DIR` / `GIT_WORK_TREE` は使わない
...
| `git --git-dir=...` を使う | 間違った config/index に書き込む事故が起きる | `git -C <worktree-path> ...` を使う |
