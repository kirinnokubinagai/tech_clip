...
```text
SendMessage(to: "analyst-{N}",      { type: "shutdown_request" })
SendMessage(to: "coder-{N}",        { type: "shutdown_request" })
SendMessage(to: "e2e-reviewer-{N}", { type: "shutdown_request" })
```
...
2. 該当 Issue の team config エントリ（`*-{N}` 形式）を列挙
...
team config に `{role}-{N}` が残っている → そのまま継続稼働中とみなす。同名 spawn は禁止（重複の原因）。
