...
ユーザー ⇄ orchestrator   (Issue 登録・割り振り・進捗報告は対話で行う)
   ...
   analyst ─→ 実装系 (coder / infra-engineer / ui-designer) ─→ impl-ready
                              ...
                              └─ E2E 影響あり → shard 実行 → aggregator → e2e-approved
...
| **e2e-reviewer** | E2E 影響あり判定 | 全 lane 揃うまで集約待機 → `harness-e2e-shard-execution`（4-shard / disk 逼迫時 2-shard） |
...
[orchestrator] harness-orchestrator-self-audit → harness-spawn-flow
   ...
   Agent(e2e-reviewer, name="issue-123-e2e-reviewer", shard_total=4, ..., mode="acceptEdits")
...
[e2e-reviewer] フェーズ 0: bash scripts/gate/evaluate-paths.sh で E2E 必要性判定
   ...
   │     （maestro 起動なし、即短絡）
   └─ e2e_required: true
       → harness-e2e-shard-execution → 全 shard PASS → aggregator → .e2e-passed
