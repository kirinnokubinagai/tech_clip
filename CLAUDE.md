...
- **zone 衝突する Issue を並列 spawn しない**: `harness-proactive-issue-triage` が `blocked_by_active=true` を返す Issue は、ブロック元 Issue がマージされるまで spawn 保留する。判定ロジックは `.claude/config.json` の `conflict_zones`、active zone は `git worktree list` ベースで自動算出（`scripts/skills/list-active-zones.sh`）。spawn-prepare.sh が exit 1 を返した場合は強行 spawn 禁止。
