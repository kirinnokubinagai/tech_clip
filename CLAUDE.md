...
1. **SessionStart 時**
   ...
   生存確認 ping に応答がなかった既存サブエージェントを再 spawn したい場合は、必ず `AskUserQuestion` で人間ユーザーに確認してから行うこと。独断での再 spawn は禁止。
...
- **reviewer を新規 spawn する前に、必ず既存の `issue-{N}-reviewer` に `SendMessage` で ping を送り応答を確認する。応答があれば既存 reviewer に `impl-ready` を送ること。応答がない場合のみ再 spawn する**（確認なしの再 spawn は `issue-{N}-reviewer-2`, `-3` … と重複を生む原因になる）
...
1. **`active-issues` チームの存在確認と既存メンバーの生存確認**
   ...
   - チームが既に存在する場合は、以下の手順で既存メンバーを再利用するか判定する:
     1. `~/.claude/teams/active-issues/config.json` を読み、既存メンバー一覧（`issue-{N}-{role}` 形式のサブエージェント名）を取得する
     2. 各既存メンバーに `SendMessage(to: "issue-{N}-{role}", "ping: session restart — alive?")` を送り生存確認する
     3. **応答あり**（60秒以内に pong / 作業中報告が返る）→ そのサブエージェントはそのまま継続稼働させる。同名で再 spawn してはならない
     4. **応答なし**（タイムアウトまたはプロセス不在）→ そのサブエージェントは本当に死亡しているので、同名で再 spawn してよい
...
3. **既存メンバーの継続稼働をユーザーに報告する**
   - 生存確認の結果、継続稼働中の `issue-{N}-{role}` が存在する場合は「前回セッションから継続稼働中のサブエージェント: issue-N-analyst / issue-N-coder / issue-N-reviewer（計 X 体）」とユーザーに一覧提示する
   - 応答がなく再 spawn したサブエージェントがある場合は「死亡確認により再 spawn: issue-N-coder」と併せて報告する
...
| `PING` | orchestrator → サブエージェント | 生存確認 |

### 10 分無音時の生存確認 ping

reviewer 系サブエージェントから **10 分以上** 任意のメッセージが届かない場合、orchestrator は生存確認 ping を送る。
...
各 reviewer サブエージェントごとに独立してタイマー管理する（複数 Issue 並列処理時に他 Issue のタイマーと混同しない）。
...
`.claude/.review-passed` は **reviewer 系サブエージェントのみが** Write ツールで作成すること（例: Write ツールで `{worktree}/.claude/.review-passed` を作成、内容は空でよい）。coder 系サブエージェントおよび orchestrator はこのマーカーを作成してはならない。
