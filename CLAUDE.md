...
| **サブエージェント** | orchestrator が `Agent` ツールで spawn する独立プロセス。`{role}-{N}` 形式の名前を持つ。spawn 元の orchestrator にしか SendMessage できない | （個別の role 名で呼ぶときも「サブエージェント」と総称してよい） |
...
[orchestrator] harness-orchestrator-self-audit → harness-spawn-flow
   ...
   [4 体セット同時 spawn]
   Agent(analyst,      name="analyst-123",      ..., mode="acceptEdits")
   Agent(coder,        name="coder-123",        ..., mode="acceptEdits")
   Agent(e2e-reviewer, name="e2e-reviewer-123", shard_total=2, ..., mode="acceptEdits")
   Agent(reviewer,     name="reviewer-123",     ..., mode="acceptEdits")
   ↓
[analyst] brainstorming → spec を docs/superpowers/specs/ に保存
   → SendMessage("spec: <path>") to coder-123
   ↓
[coder] impl-wait-for-spec → test-driven-development → impl-lint-commit-notify
   → SendMessage("impl-ready: <hash>") to e2e-reviewer-123  ← 必ず e2e-reviewer
   ↓
[e2e-reviewer] フェーズ 0: bash scripts/gate/evaluate-paths.sh で E2E 必要性判定
   ├─ e2e_required: false
   │   → SendMessage("e2e-approved: <hash>") to reviewer-123 + shutdown
   │     （maestro 起動なし、即短絡）
   └─ e2e_required: true
       → harness-e2e-shard-execution（Maestro native parallel + per-flow 監視） → .e2e-passed
       → SendMessage("e2e-approved: <hash>") to reviewer-123
   ↓
...
- **orchestrator はソースファイル・スクリプト・テストを直接編集しない**（main でも worktree でも禁止）。実装・修正はすべて実装系サブエージェントに委譲する。orchestrator が直接編集してよいのは `.claude/**`（skill/agent/hook 定義）・`CLAUDE.md`・memory ファイルのみ（`orchestrator-direct-edit-guard.sh` が物理強制）
