...
4. coder と reviewer を background spawn する
5. `/tmp/tech-clip-issue-{N}/pr-url` をポーリングし PR URL を取得する
6. ScheduleWakeup で GitHub レビューをポーリングし、修正ループを回す
...
③ [並列 background spawn]
   ...
   - coder は spec を読んで実装 → /tmp/coder-ready に書く
   - reviewer は coder-ready を待ってレビュー → /tmp/review-result.json に書く
   - PASS になったら reviewer が push + PR 作成 → /tmp/pr-url に書く

④ /tmp/tech-clip-issue-{N}/pr-url をポーリング
   - PR URL 取得 → ScheduleWakeup(300秒, "PR #{N} GitHub レビュー確認")

── GitHub レビュー待機ループ ──
⑤ 起きたら: gh pr view {PR番号} --json reviews で確認
   - PENDING          → ScheduleWakeup(300秒) でまた眠る
   - APPROVED         → ユーザーに完了報告 → 終了
   - CHANGES_REQUESTED →
       レビューコメントを読む
       /tmp/tech-clip-issue-{N}/coder-ready と review-result.json を削除（リセット）
       Agent(coder,    run_in_background=true, mode="acceptEdits")（feedback を prompt に含める）
       Agent(reviewer, run_in_background=true, mode="acceptEdits")（PR番号を渡す）
...
② Agent(analyst, mode="acceptEdits")（設計・完了を待つ）

③ Agent(infra-engineer, mode="acceptEdits")（実装・完了を待つ）

--- レビューループ ---
④ [並列 spawn] Agent(infra-reviewer, mode="acceptEdits")
               Agent(reviewer, mode="acceptEdits")（セキュリティ観点）
   両方 PASS → マーカー作成 → bash scripts/push-verified.sh → PR 作成
...
③ Agent(ui-designer, mode="acceptEdits")（実装・完了を待つ）

--- レビューループ ---
④ [並列 spawn] Agent(ui-reviewer, mode="acceptEdits")
               Agent(reviewer, mode="acceptEdits")
   両方 PASS → マーカー作成 → bash scripts/push-verified.sh → PR 作成
   指摘あり  → Agent(ui-designer) で修正 → ④へ戻る
```
...
Step 2 の GitHub レビュー待機ループ（⑤）で ScheduleWakeup を使って処理する（オーケストレーターのコンテキストを消費しない）。
...
|------|-----------|
| `APPROVED` | 完了。ユーザーに報告 |
| `CHANGES_REQUESTED` | レビューコメントを読む → /tmp/ リセット → coder + reviewer を background spawn → pr-url をポーリング → ⑤ に戻る |
| `PENDING` | ScheduleWakeup(300秒) で再確認 |

> **注意**: `pre-push-review-guard.sh` により、`.claude/.review-passed` マーカーがない状態での push はブロックされる。reviewer エージェントがマーカー作成・push・PR 作成を担当する。
...
- **reviewer が PASS するまで push しない**（CRITICAL / HIGH / MEDIUM / LOW 問わず指摘が 1 件でも残れば修正ループを続ける）
...
- **レビュー PASS 後のマーカー作成・push・PR 作成は reviewer エージェントが担当する**（オーケストレーターは行わない）
