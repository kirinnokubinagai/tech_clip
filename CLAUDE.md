...
ユーザー ⇄ orchestrator   (Issue 登録・割り振り・進捗報告は対話で行う)
         ...
         Issue ごとに worktree 作成
                ↓
   [同一メッセージで 4 体セット background spawn]
   analyst ─→ 実装系 (coder / infra-engineer / ui-designer) ─→ impl-ready
                                                              ↓
                              e2e-reviewer（常に存在、4 体セットの 1 つ）
                              ├─ E2E 影響なし → 即 e2e-approved 転送 + shutdown
                              │                  （evaluate-paths.sh で判定、maestro なし）
                              └─ E2E 影響あり → Maestro native parallel 実行 → e2e-approved
                                                 不合格なら CHANGES_REQUESTED → 実装系へ
                                                              ↓
...
- **e2e-reviewer は常時 spawn**（条件付きではない、4 体セット固定）
- E2E 影響なしの判定は e2e-reviewer 自身が `evaluate-paths.sh` で行い、不要なら短絡（maestro 起動なし）
- 実装系は **必ず e2e-reviewer に impl-ready を送る**（reviewer に直接送らない）
- reviewer は常に **`e2e-approved`** を待つ（実装系から直接 `impl-ready` は来ない）
...
| **coder / infra-engineer / ui-designer** | `spec:` 受信 | `test-driven-development` → 実装 → `impl-lint-commit-notify` → **必ず e2e-reviewer に impl-ready 送信**（reviewer に直接送らない） |
...
| **e2e-reviewer** | `impl-ready: ...` 受信 | フェーズ 0: `evaluate-paths.sh` で E2E 必要性判定 |
| **e2e-reviewer** | E2E 影響なし判定 | 即 `e2e-approved: <hash>` を reviewer に転送 + shutdown（maestro なし、短絡） |
| **e2e-reviewer** | E2E 影響あり判定 | `harness-e2e-shard-execution` skill（Maestro native parallel + per-flow 監視ループ） |
| **e2e-reviewer** | 全 flow PASS | `.e2e-passed` 生成 → reviewer へ `e2e-approved` |
| **e2e-reviewer** | flow FAIL | coder へ `CHANGES_REQUESTED:` 送信 |
| **reviewer / infra-reviewer / ui-reviewer** | `e2e-approved:` 受信 | `review-push-validation` → `review-conflict-check` → `review-pre-check` → `review-code-review` → `review-push-and-pr` → `review-polling-wait` |
...
[orchestrator] harness-orchestrator-self-audit → harness-spawn-flow
   ...
   Agent(e2e-reviewer, name="issue-123-e2e-reviewer", shard_total=2, ..., mode="acceptEdits")
...
[coder] impl-wait-for-spec → test-driven-development → impl-lint-commit-notify
   → SendMessage("impl-ready: <hash>") to issue-123-e2e-reviewer  ← 必ず e2e-reviewer
   ↓
[e2e-reviewer] フェーズ 0: bash scripts/gate/evaluate-paths.sh で E2E 必要性判定
   ├─ e2e_required: false
   │   → SendMessage("e2e-approved: <hash>") to issue-123-reviewer + shutdown
   │     （maestro 起動なし、即短絡）
   └─ e2e_required: true
       → harness-e2e-shard-execution（Maestro native parallel + per-flow 監視） → .e2e-passed
       → SendMessage("e2e-approved: <hash>") to issue-123-reviewer
   ↓
[reviewer] review-push-validation, review-conflict-check, review-pre-check, review-code-review, review-push-and-pr, review-polling-wait
...
- 破壊的な Git コマンドを使わない
- **Branch 戦略**: `feature/* → stage → main`
  - 通常開発: `git checkout -b feature/<desc> origin/stage` で worktree 作成
  - PR は `stage` をターゲットにする (`gh pr create --base stage`)
  - main への直接 push 禁止 (branch protection)
  - reviewer 系の `.review-passed` / `.e2e-passed` marker は stage → main の最終 PR でのみ必須
  - feature/* / issue/* 等の branch から push 時は marker check が自動 skip (CI gate に委譲)
...
- **claude-review bot の指摘は重要度問わず全件修正必須**（💡 改善提案 / Nitpick 1 件でも残れば push 不可）。「提案だから無視してよい」は禁止
- **`AI Review: NEEDS WORK` / `AI Review: APPROVED` / `AI Review: SKIPPED` ラベルの手動 add / remove は禁止**（`dangerous-command-guard.sh` が物理強制）。ラベルは bot が自動更新する。修正したい場合は bot の指摘を解消して push し直す

### orchestrator 必須事項（「土管」原則）

**orchestrator は spawn 後、analyst にのみ SendMessage する。coder / infra-engineer / ui-designer / e2e-reviewer / reviewer への直接 SendMessage は禁止。**

指示の流れ: `ユーザー → orchestrator → analyst → 各種 engineer → e2e-reviewer → reviewer → GitHub CI`
質問の流れ: `各種 engineer / e2e-reviewer / reviewer → analyst → orchestrator → ユーザー`

orchestrator は「〇〇してください」「Issue #N をお願いします」と analyst に伝えるだけ。HOW は analyst が決める。orchestrator が技術詳細に踏み込まない（コンテキスト節約）。
...
- **spawn 後の SendMessage は analyst 宛のみ**（`orchestrator-flow-guard.sh` が物理強制）。例外なし。
...
- **orchestrator はソースファイル・スクリプト・テスト・`.claude/**`・`CLAUDE.md`・`flake.nix` のいずれも直接編集しない**（main でも worktree でも禁止）。実装・修正・ハーネス改善はすべて Issue + worktree + 実装系サブエージェント経由で PR を作成する。orchestrator が直接編集してよいのは memory ファイル (`.claude-user/**`) と `.omc/**`（gitignore済み実行状態ファイル）のみ（`orchestrator-direct-edit-guard.sh` が物理強制）
...
### 改善要請に対する恒久対応
orchestrator またはサブエージェントの挙動について指摘を受けた場合、memory への記録だけで終わらせず、`harness-self-improvement` skill を呼んで Issue を立て、worktree + 実装系サブエージェント経由で PR を作成する。**main で `.claude/**` / `CLAUDE.md` を直接編集することは禁止**（`orchestrator-direct-edit-guard.sh` が物理強制）。
...
| [`harness-e2e-shard-execution`](.claude/skills/harness-e2e-shard-execution/SKILL.md) | E2E Maestro native parallel 実行 + per-flow 監視 | e2e-reviewer |
| [`harness-standard-flow-discipline`](.claude/skills/harness-standard-flow-discipline/SKILL.md) | 標準フロー逸脱時の bubble-up 手順 | 全サブエージェント |
| [`harness-self-improvement`](.claude/skills/harness-self-improvement/SKILL.md) | フロー違反 / 改善点を検知した時の Issue 化 + spawn-flow 起動 | orchestrator |
...
| `impl-await-feedback` | CHANGES_REQUESTED / CONFLICT_RESOLVE 受信待機ループ |
...
- harness 中核 skill: name に `harness-` プレフィックスを持つ skill 群（`.claude/skills/harness-*/SKILL.md`）
